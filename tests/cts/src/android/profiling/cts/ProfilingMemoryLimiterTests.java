...
public class ProfilingMemoryLimiterTests {
    private static final String TAG = ProfilingMemoryLimiterTests.class.getSimpleName();
    ...
    public void setup() {
        ...
        executeShellCmd(
                "device_config put profiling_testing"
                    + " system_triggered_profiling.testing_package_name"
                    + " android.profiling.cts.profilingapp");
        ...
        // even after the process that created them has exited.
        startActivityWithAction(ACTION_INIT_AND_ADD_ANOMALY_TRIGGER);
    ...
    public void cleanup() {
        resetAllConfigs();
        executeShellCmd(
                "device_config delete profiling_testing"
                    + " system_triggered_profiling.testing_package_name");
    ...
    public void testMemoryAnomalyTrigger() throws Exception {
        ...
        mResultReceiverFilter =
                new ResultReceiverFilter(
                        REPLY_ACTION_COMPLETE, /* resultsToWaitFor= */ 1, /* timeoutMs= */ 120_000);

        // Start the test app to register and allocate memory.
        ...
        expect.that(result.getResultFilePath()).contains(OUTPUT_FILE_JAVA_HEAP_DUMP_SUFFIX);
        expect.that(result.getTriggerType()).isEqualTo(ProfilingTrigger.TRIGGER_TYPE_ANOMALY);
        // Error message might be present, so we don't assert it is null.
